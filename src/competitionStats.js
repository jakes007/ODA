...
function getCompetitionTeams(registry, competitionId, division = '') {
  return Object.values(registry.teams).filter((team) => {
    ...
    return (
      cleanString(team.division).toLowerCase() ===
      cleanString(division).toLowerCase()
    );
  });
}
...
export function buildCompetitionStandings(registry, competitionId, options = {}) {
  ...
  const divisionFilter = cleanString(options.division);

  const teamResults = Object.values(registry.historicalTeamResultsNormalized || {})
    .filter((result) => result.competitionId === competitionId)
    .filter((result) => {
      if (!divisionFilter) return true;

      return (
        cleanString(result.division).toLowerCase() ===
        divisionFilter.toLowerCase()
      );
    })
    .filter((result) => cleanString(result.teamName));
  ...
  teamResults.forEach((result) => {
    ...
    row.played += toNumber(result.metrics?.gp);
    row.won += toNumber(result.metrics?.wins);
    row.drawn += toNumber(result.metrics?.draws);
    row.lost += toNumber(result.metrics?.losses);
    row.leaguePoints += toNumber(result.metrics?.points);
    row.matchPointsFor += toNumber(result.metrics?.legsWon);
    row.matchPointsAgainst += toNumber(result.metrics?.legsLost);
    row.legsWon += toNumber(result.metrics?.legsWon);
    row.legsLost += toNumber(result.metrics?.legsLost);
  });
